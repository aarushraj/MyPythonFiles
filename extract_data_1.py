...
def extract_data_from_excel(excel_file):
    ...
    try:
        ...
        data = {
            "StoryName": get_cell_value(doc_control_sheet, 'StoryName'),
            "GWID": get_cell_value(doc_control_sheet, 'GWID'),
            "Description": get_cell_value(doc_control_sheet, 'Description'),
            "StoryPoints": get_cell_value(doc_control_sheet, 'StoryPoints'),
            "StoryAssumptions": get_cell_value(doc_control_sheet, 'StoryAssumptions')
        }
    except KeyError:
        data = {}
    workbook.close()
...
def get_cell_value(sheet, cell_name):
    """Gets the value of a named cell in a worksheet, or returns 'Not found' if not found."""
    try:
