...
def extract_data_from_excel(excel_file):
    ...
    data = {
        "Theme": get_named_cell_value(workbook, 'Theme', doc_control_sheet),
...
def get_named_cell_value(workbook, range_name, sheet):
    ...
    named_range = workbook.defined_names.get(range_name)
    if named_range:
        for range in named_range.destinations:
            dest_sheet, dest_range = range
            if dest_sheet == sheet.title:
                value = sheet[dest_range].value
                return value
    return 'Not found'
...
def write_output_to_excel(output_file, data_list):
    ...
    # Write header row
    header = ["SourceExcel", "Theme", "StoryName", "GWID", "Description", "StoryPoints", "StoryAssumptions"]
